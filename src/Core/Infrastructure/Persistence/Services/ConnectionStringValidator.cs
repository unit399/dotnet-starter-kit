...
namespace ROC.Core.Infrastructure.Persistence.Services;

internal sealed class ConnectionStringValidator(
    ...
    ILogger<ConnectionStringValidator> logger) : IConnectionStringValidator
{
    ...
    {
        if (string.IsNullOrWhiteSpace(dbProvider)) dbProvider = _dbSettings.Provider;
