...
namespace ROC.Core.Infrastructure.Tenant.Services;

public sealed class TenantService : ITenantService
{
    private readonly DatabaseOptions _config;
    private readonly IServiceProvider _serviceProvider;
    private readonly IMultiTenantStore<RocTenantInfo> _tenantStore;

    public TenantService(IMultiTenantStore<RocTenantInfo> tenantStore, IOptions<DatabaseOptions> config,
    ...
    public async Task<string> ActivateAsync(string id, CancellationToken cancellationToken)
    {
        ...
        if (tenant.IsActive) throw new BaseException($"tenant {id} is already activated");
    ...
    public async Task<string> CreateAsync(CreateTenantCommand request, CancellationToken cancellationToken)
    {
        ...
        if (request.ConnectionString?.Trim() == _config.ConnectionString.Trim()) connectionString = string.Empty;

        RocTenantInfo tenant = new(request.Id, request.Name, connectionString, request.AdminEmail, request.Issuer);
    ...
    {
        var tenant = await GetTenantInfoAsync(id).ConfigureAwait(false);
        if (!tenant.IsActive) throw new BaseException($"tenant {id} is already deactivated");
    ...
    {
        return await _tenantStore.TryGetAsync(id).ConfigureAwait(false) is not null;
    }

    public async Task<bool> ExistsWithNameAsync(string name)
    {
        return (await _tenantStore.GetAllAsync().ConfigureAwait(false)).Any(t => t.Name == name);
    }
    ...
    {
        return (await GetTenantInfoAsync(id).ConfigureAwait(false))
            .Adapt<TenantDetail>();
    }
    ...
    private async Task InitializeDatabase(RocTenantInfo tenant)
    {
    ...
    private async Task<RocTenantInfo> GetTenantInfoAsync(string id)
    {
        return await _tenantStore.TryGetAsync(id).ConfigureAwait(false)
               ?? throw new NotFoundException($"{typeof(RocTenantInfo).Name} {id} Not Found.");
    }
}