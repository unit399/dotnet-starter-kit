...
using ROC.Core.Infrastructure.Tenant.Interfaces;
using ROC.WebApi.Core.Exceptions;
using ROC.WebApi.Core.Tenant;

namespace ROC.Core.Infrastructure.Tenant;

public class RocTenantInfo : IRocTenantInfo
{
    public RocTenantInfo()
    {
    }

    public RocTenantInfo(string id, string name, string? connectionString, string adminEmail, string? issuer = null)
    {
        Id = id;
        Identifier = id;
        Name = name;
    ...
    string? IRocTenantInfo.ConnectionString
    {
    ...
    public void SetValidity(in DateTime validTill)
    {
        ValidUpto = ValidUpto < validTill
            ? validTill
            : throw new BaseException("Subscription cannot be backdated.");
    }

    public void Activate()
    {
        if (Id == TenantConstants.Root.Id) throw new InvalidOperationException("Invalid Tenant");
